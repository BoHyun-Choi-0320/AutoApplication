...
class MyApp(QWidget):
    ...
    def setGridLayout(self):
        ...
        # 객체 생성
        self.filePath = QLineEdit('E:/geostory/2023/타 부서 업무협조/해양사업부/데이터/자동화프로그램구축_총괄표_(신양식) 장애물 관리대장(무안공항)_231211.xlsx')
    ...
    # 관리대장 읽어오는 함수
    def readExcel(self):
        self.pf = pd.read_excel(self.filePath.text(), header=3, usecols='B,C,E,I:L,P:AF,AJ,AL,AN,AP,AQ,AZ,BA,BH,BZ')
        self.pf.columns = ['순번', '신규연도', '연번', '세부종류', '장애물 용도', '명칭', '건축주(소유주)', '특례 장애물 구분', '차폐 기준 장애물 및 지정일',
                           '주소1', '주소2',
                           '주소3',
                           '주소4', '도로명주소', '위치구역', '위도', '경도', 'X축', 'Y축', '지반높이(DEM)', '건물/시설물/수목 높이', '전체높이(DSM)',
                           '제한표면',
                           '제한표면 침범높이',
                           '협의높이', '위반여부', '기관명', '관리번호', '연락처', '건축허가일', '준공승인일', '장애물 등재일', '좌표/높이 결정방법']
        self.makeFolder()
        self.copyExcel()
    ...
    def inputDataToExcel(self, savePath, data, imgPath):
        try:
            ...
            ws1['A13'].value =data['지반높이(DEM)']
            ws1['B13'].value =data['건물/시설물/수목 높이']
            ws1['C13'].value =data['전체높이(DSM)']
